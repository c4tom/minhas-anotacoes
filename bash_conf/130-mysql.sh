...
ct_mysql_adm_AddUserOnlyDataAccess() {
	...
	local MY_HOST=$2
	local MY_PASS=$3
	echo "CREATE USER '$MY_USER'@'$MY_HOST' IDENTIFIED VIA mysql_native_password USING '$MY_PASS';GRANT SELECT, INSERT, UPDATE, DELETE, FILE ON *.* TO '$MY_USER'@'$MY_HOST' REQUIRE NONE WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;"
}
...
ct_mysql_addUserCreateTableGivePermission() {
   ...
   local database=${4:-"mydb"};

	echo "CREATE USER '$user'@'$host' IDENTIFIED VIA mysql_native_password USING password('$passwd');
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, FILE, INDEX, ALTER, CREATE TEMPORARY TABLES, CREATE VIEW, EVENT, TRIGGER, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, EXECUTE ON '$database'.* TO '$user'@'$host' REQUIRE NONE WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;
